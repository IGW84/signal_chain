...
class ADC:
    ...
    def __repr__(self):
        s = f'name: {self.name}\n'
        s += f'\tbits : {self.bits}\n'
        s += f'\ttemp drift (ppm/degC): {self.gain_drift_temp:.3f} time drift (ppm/1000hr): {self.gain_drift_time:.3f}\n'
    ...
    def sample(self, vin, vref, temp=None, time=None):
        ...
        gain = gain_nom * (1 + gain_error_ppm/1e6)
        # offset
...
if __name__ == '__main__':
    ...
    temperature = 25
    cols = ['run', 'offset','gain drift temperature', 'gain drift time',
